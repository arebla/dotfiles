...
class Py3status:
    ...
    mem_threshold = 80
#
    cpu_format = "<span foreground='#81a1c1' rise='-1000'></span> <span foreground='#fdf6e3'>{cpu_used_percent:.2f}%</span>  "
    mem_format = "<span foreground='#81a1c1' rise='-1000'></span> <span foreground='#fdf6e3'>{mem_used_percent:.2f}%</span>"
    ...
    def system_data(self):
        ...
        cpu_usage = sys.get("cpu_used_percent", 0)
        mem_usage = sys.get("mem_used_percent", 0)

        format_string = ""
    
        if cpu_usage > self.cpu_threshold:
            format_string += self.cpu_format

        if mem_usage > self.mem_threshold:
            format_string += self.mem_format
    
        return {
            "cached_until": self.py3.time_in(self.cache_timeout),
            "full_text": self.py3.safe_format(format_string, sys),
        }
